...
describe SimpleMessages::Builder do
  ...
  it { expect(@builder.header_wrapper).to eq '<strong>This title my alert message.</strong><br>' }
  ...
  context "without close" do
    ...
    it { expect(@builder.to_html).to eq "<div class='alert alert-success' role='alert'><strong>This title my alert message.</strong><br>Lorem Ipsum is simply dummy text of the printing and typesetting industry.</div>" }
  end
  ...
  context "body is a array" do
    ...
    it { expect(@builder.to_html).to eq "<div class='alert alert-success alert-dismissible' role='alert'><button type='button' class='close' data-dismiss='alert'><span aria-hidden='true'>&times;</span><span class='sr-only'>Close</span></button><strong>This title my alert message.</strong><br>Line 1.<br />Line 2.</div>" }
  end
